...
void data_init() {
    ...
    data = (uint8_t*)malloc(DECODE_BUFFER_SIZE * NUM_CLIENTS);

    FILE* pfile;
    size_t file_size, client_size, size;

    sprintf(data_filename, "data_0");
    pfile = fopen(data_filename, "rb");
    fseek(pfile, 0L, SEEK_END);
    file_size = ftell(pfile) * COLUMN;
    fclose(pfile);
    sprintf(data_filename, "data");

    if (file_size % ENCODE_BLOCK_SIZE) {
        printf("File size is not aligned\n");
        assert(0);
    }
    ...
    printf("File size == %lu \n", file_size);
    printf("Decode block size == %lu \n", ENCODE_BLOCK_SIZE);
    for (i = 0; i < NUM_CLIENTS; ++i)
        printf("Fragment %d: offset == %lu, size == %lu \n", i, jobs[i].data_offset, jobs[i].data_size);

    for (i = 0; i < NUM_CLIENTS; ++i) {
        *(uint64_t*)(data + DECODE_BUFFER_SIZE * i) = htonll(jobs[i].data_size);
        *(uint64_t*)(data + DECODE_BUFFER_SIZE * i + sizeof(uint64_t)) = htonll(jobs[i].data_offset);
        strcpy(data + DECODE_BUFFER_SIZE * i + 2 * sizeof(uint64_t), data_filename);
        jobs[i].data_ptr = data + DECODE_BUFFER_SIZE * i;
    }

    for (i = 0; i < ROW; ++i) {
        sprintf(data_filename, "data_%d", i); 
        pfile = fopen(data_filename, "rb");
        for (j = 0; j < NUM_CLIENTS; ++j) {
            if (jobs[j].data_size == 0) continue;
            fseek(pfile, jobs[j].data_offset / COLUMN, SEEK_SET);
            size = fread(jobs[j].data_ptr + jobs[j].data_size / COLUMN * i + BUFFER_HEADER_SIZE,
                         jobs[j].data_size / COLUMN,
                         1, pfile);
            assert(size == 1);
        }
        fclose(pfile);
...
void network_init() {
    ...
    for (i = 0; i < NUM_CLIENTS; ++i) {
        ...
        mr_data[i] = ibv_reg_mr(pd[i], data + DECODE_BUFFER_SIZE * i, 
                                DECODE_BUFFER_SIZE, 
...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < NUM_CLIENTS; ++i) {
        ...
        jobs[i].timeout = time(0) + calc_timeout(jobs[i].data_size / COLUMN * ROW);
        post_send_data(i, jobs[i].data_ptr, BUFFER_HEADER_SIZE + jobs[i].data_size / COLUMN * ROW);
    }
    ...
    while (1) {
        ...
        if (ack_id == 0) {
            ...
            if (i < NUM_CLIENTS && j < NUM_CLIENTS) {
                ...
                memcpy(jobs[j].data_ptr, jobs[i].data_ptr, BUFFER_HEADER_SIZE + jobs[j].data_size / COLUMN * ROW);
                jobs[j].finished = 0;
                jobs[j].broken = 0;

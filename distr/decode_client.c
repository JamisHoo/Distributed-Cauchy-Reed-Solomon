...
void data_init() {
    data = (uint8_t*)malloc(DECODE_BUFFER_SIZE);
    decoded = (uint8_t*)malloc(BUFFER_SIZE);
...
void network_init() {
    ...
    mr_data = ibv_reg_mr(pd, data, DECODE_BUFFER_SIZE, 
    ...
    sge_data.length = DECODE_BUFFER_SIZE;
...
void handle_data() {
    ...
    */
    ...
    for (i = 0; i < ROW; ++i) {
        output[i] = data + BUFFER_HEADER_SIZE + data_size / COLUMN * i;
        row[i] = i;
    }

    size = ec_method_parallel_decode(data_size / COLUMN, COLUMN, row, output, decoded, get_nprocs());
    assert(size == data_size);

    pfile = fopen(filename, "rb+");
    fseek(pfile, data_offset, SEEK_SET);
    size = fwrite(decoded, data_size, 1, pfile);
    assert(size == 1);
    fclose(pfile);
}
